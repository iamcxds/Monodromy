...
move atl {chartId,pos} dir =
    let 
        ...
        --target Position
        tP= pAdd (d2V dir) pos
        notMoveResult = Ok (False, (GlobalPos chartId pos))    
    in 
    case mcha of
        Nothing-> Err "Can not find the chart"
        Just cha ->
...
visableAreaByLine atl gPs line =
    ...
    in 
        case moveSimple (List.head gPs, List.head line) of
            (False,_) -> gPs
            (True, newGP) -> visableAreaByLine atl (newGP++gPs) (myTail line)
